...
namespace ZoneTop.Application.SSO.Common.Utils
{
    ...
    {
        //redis
        private static LogModel log = LogUtils.GetLogger("RedisCacheUtils");
        #region -- 连接信息 --
        /// <summary>
        /// redis配置文件信息
        /// </summary>
        private static RedisConfigModel redisConfigInfo = RedisConfigModel.GetConfig();
        private static PooledRedisClientManager prcm;
        /// <summary>
        /// 静态构造方法，初始化链接池管理对象
        /// </summary>
        static RedisCacheUtils()
        {
            CreateManager();
        }
        /// <summary>
        ...
        /// <param name="key"></param>
        /// <param name="t"></param>
        /// <param name="timeSpan">为null时,使用用户配置,用户未配置默认600s</param>
        /// <returns></returns>
        public static bool Set<T>(string key, T t, TimeSpan? timeSpan)
        {
            ...
            if (timeSpan == null)
            {
                timeSpan = new TimeSpan(GrobalConfig.RedisTimeOut);
            }
            using (IRedisClient redis = prcm.GetClient())
            {
                return redis.Set<T>(key, t, (TimeSpan)timeSpan);
            }
        }
        ...
        public static void TestWrite()
        {
            ...
            {
                Set<string>(i.ToString(), i.ToString(), null);
            }
        }
        #endregion
    }
